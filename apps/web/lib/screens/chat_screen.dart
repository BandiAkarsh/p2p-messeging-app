...
class ChatScreen extends ConsumerStatefulWidget { // Chat screen with Riverpod
...
class _ChatScreenState extends ConsumerState<ChatScreen> { // Chat screen state
  final TextEditingController _messageController = TextEditingController(); // Input controller
  final String _currentPeerId = 'peer_123'; // Current peer (mock)

  @override
  Widget build(BuildContext context) { // Build UI
    final messagesAsync = ref.watch(conversationProvider(_currentPeerId)); // Watch messages

    return Scaffold( // Scaffold layout
      appBar: AppBar( // App bar
        title: const Text('EcoMesh Chat'), // Title
        backgroundColor: Theme.of(context).colorScheme.inversePrimary, // Themed background
        actions: [ // Action buttons
          IconButton( // AI suggestions button
      ...
      body: Column( // Vertical layout
        children: [ // Children widgets
          Expanded( // Flexible space
            child: messagesAsync.when( // Handle async state
              data: (messages) => _buildMessageList(messages), // Build message list
              loading: () => const Center(child: CircularProgressIndicator()), // Loading
              error: (error, stack) => Center(child: Text('Error: $error')), // Error display
            ),
          ),
          MessageInput( // Message input widget
  ...
  Widget _buildMessageList(List<Message> messages) { // Build message list
    return ListView.builder( // Scrollable list
      ...
      itemBuilder: (context, index) { // Item builder
        final message = messages[messages.length - 1 - index]; // Get message (reversed)
        final isMe = message.senderId != _currentPeerId; // Check if sent by me

        return MessageBubble( // Message bubble widget
  ...
  Future<void> _sendMessage() async { // Send message handler
    ...
    await messagingService.sendMessage(                               // Send message
  ...
  Future<void> _replyToMessage(Message message) async {               // Reply to message
    _messageController.text = '> ${message.content}\n';               // Quote original
  }

  Future<void> _getAiSuggestion() async {                             // Get AI suggestion
    final aiService = ref.read(aiServiceProvider);                    // Get AI service
    final messagingService = ref.read(messagingServiceProvider);      // Get messaging
    final messages = await messagingService.getConversation(_currentPeerId); // Get history

    if (messages.isEmpty) return;                                     // No history

    final lastMessage = messages.last; // Get last message
    final aiContext = MessageContext(                                   // Build AI context
      incomingMessage: lastMessage.content, // Last message
      recentHistory: messages.take(5).map((m) => m.content).toList(), // Recent 5 messages
      preferredTone: 'friendly',                                      // Friendly tone
    );

    try {
      final suggestion = await aiService.generateReply(aiContext); // Get AI reply
      _messageController.text = suggestion; // Fill input
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(this.context).showSnackBar( // Show error
  ...
  void _showSmartReplies() {                                          // Show smart reply options
    showModalBottomSheet(                                             // Show bottom sheet
      ...
      builder: (context) => Container( // Container
        ...
        child: Column( // Vertical layout
          ...
          children: [                                                 // Children
            const Text('Smart Replies', style: TextStyle(             // Title
  ...
  Widget _buildReplyChip(String text) { // Build reply chip
    return Padding(                                                   // Padding wrapper
      padding: const EdgeInsets.symmetric(vertical: 4),               // Vertical padding
      child: ActionChip(                                              // Action chip
