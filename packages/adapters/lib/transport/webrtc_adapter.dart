...
class WebRTCAdapter implements ITransportPort {
  ...
  Future<void> connect(String peerId) async {
    ...
    try {
      final wsUrl =
          '$signalingUrl/ws?room=global&peer=$peerId'; // Build WebSocket URL
      _channel =
          WebSocketChannel.connect(Uri.parse(wsUrl)); // Connect WebSocket
      ...
      _channel!.stream.listen(
        ...
        onDone: () =>
            _status = TransportStatus.disconnected, // Handle disconnect
      );
  ...
  Future<void> send(EncryptedMessage message) async {
    // Send message
    if (_channel == null || _status != TransportStatus.connected) {
      // Check connection
  ...
  void _handleMessage(String message) {
    ...
    try {
      ...
      final encryptedMessage = EncryptedMessage(
        ...
        id: data['id'] ??
            DateTime.now().toIso8601String(), // Use provided or new ID
        senderId: data['senderId'] as String, // Extract sender
        recipientId: data['recipientId'] as String, // Extract recipient
        encryptedPayload:
            base64Decode(data['encryptedData'] as String), // Decode payload
        timestamp:
            DateTime.parse(data['timestamp'] as String), // Parse timestamp
